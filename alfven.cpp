...
alfven::alfven(float deltax, float deltay, int width, int height,\
...
{uxfile.open("a_ux.txt");
    uyfile.open("a_uy.txt");
    uzfile.open("a_uz.txt");
    bxfile.open("a_bx.txt");
    byfile.open("a_by.txt");
    bzfile.open("a_bz.txt");
    rhofile.open("a_rho.txt");
    pfile.open("a_p.txt");}

//alfven wave
//secondary closure relation not implemented
void alfven::initialize_grid() {
  for (int j = 0; j <= ny; j++) {
    for (int i = 0; i <= nx; i++) {
      here = index(i,j),
      rho->set(here,0.5);
      ...
      p->set(here,0.5);
      h->set(here,pow(p->get(here)/2.0,1.0/gamma));
      bx->set(here,1);      
      by->set(here,0);
      bz->set(here,b_guide);
      eta->set(here,res);
